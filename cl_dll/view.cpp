...
#define VIEWHEIGHT_STAND 32
#define VIEWHEIGHT_DUCK 25
#define VIEWHEIGHT_PRONE -10
// Spectator Mode
extern "C" 
{
...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	// view is the weapon model (only visible from inside body )
	view = gEngfuncs.GetViewModel();
	view->curstate.skin=(g_iUser1? g_PlayerExtraInfo[g_iUser2].teamnumber : g_iTeamNumber) -1;
	...
	angles[2]=lastlean;
	// offsets
	int leanleft,leanright,proning;
	float proneout=0.0;
	if(g_iUser1) {
		cl_entity_s *ent=gEngfuncs.GetEntityByIndex(g_iUser2);
		leanleft=ent->curstate.iuser4&U_LEANLEFT;
		leanright=ent->curstate.iuser4&U_LEANRIGHT;
		proning=ent->curstate.iuser4&U_PRONING;
		proneout=25.0;
	} else {
		leanleft=pparams->cmd->buttons&IN_LEANLEFT;
		leanright=pparams->cmd->buttons&IN_LEANRIGHT;
		proning=pparams->cmd->buttons&IN_PRONE;
		if(pparams->viewheight[2]<0.0f)
			proneout=-25*pparams->viewheight[2]/10.0f;
		
	}
//	if(proning) 
		VectorMA(pparams->vieworg,proneout,pparams->forward,pparams->vieworg);

	if(leanright&&leanleft) 
		leanright=leanleft=0;
	float maxlean;
	pmtrace_t tr;
	gEngfuncs.pEventAPI->EV_PushPMStates();
	gEngfuncs.pEventAPI->EV_SetTraceHull(2);
	gEngfuncs.pEventAPI->EV_SetSolidPlayers(
		(g_iUser1 ? g_iUser2 :gEngfuncs.GetLocalPlayer()->index)-1);
	if(leanright) {
		VectorMA(pparams->vieworg,13+8,pparams->right,pparams->forward);
		gEngfuncs.pEventAPI->EV_PlayerTrace(pparams->vieworg,pparams->forward,PM_NORMAL,-1,&tr);
		if(tr.fraction==1.0f)
			maxlean=LEAN_ANG;
		else 
			maxlean=asin(tr.fraction*sin(LEAN_ANG*M_PI/180.0))*180.0/M_PI;
		if(angles[2]<maxlean)
			angles[2]+=LEAN_RATE*LEAN_ANG*gHUD.m_flTimeDelta;
		if(angles[2]>maxlean)
			angles[2]=maxlean;
	} else if(leanleft) {
		VectorMA(pparams->vieworg,-13-8,pparams->right,pparams->forward);
	...
	VectorCopy(pparams->vieworg,pparams->simorg);
	pparams->simorg[2]-=pparams->viewheight[2]+bob;
	AngleVectors ( angles, pparams->forward, pparams->right, pparams->up );
	...
	view->origin[2] -= 1;
	vec3_t tf,tri,tu;
	AngleVectors(view->angles,tf,tri,tu);
/*	VectorMA(view->origin,CVAR_GET_FLOAT("hej_u"),tu,view->origin);
	VectorMA(view->origin,CVAR_GET_FLOAT("hej_f"),tf,view->origin);
	VectorMA(view->origin,CVAR_GET_FLOAT("hej_r"),tri,view->origin);
...
//	VectorAdd ( pparams->viewangles, (float *)&ev_punchangle, pparams->viewangles);
	...
	VectorCopy(pparams->viewangles,pparams->cl_viewangles);
...
void V_GetChasePos(int target, float * cl_angles, float * origin, float * angles)
{
	...
	else
	{
		...
		origin[2]+= VIEWHEIGHT_STAND; // DEFAULT_VIEWHEIGHT - some offset
...
void V_GetInEyePos(int target, float * origin, float * angles )
{
	...
	if(ent->curstate.usehull==2) 
		origin[2]+= VIEWHEIGHT_PRONE;
	else if (ent->curstate.usehull == 1 )
		origin[2]+= VIEWHEIGHT_DUCK; // VEC_DUCK_VIEW;
	else if(ent->curstate.usehull==0)
		...
		origin[2]+= VIEWHEIGHT_STAND; // DEFAULT_VIEWHEIGHT
}
...
int V_FindViewModelByWeaponModel(int weaponindex)
{
	...
	static char * modelmap[][2] =	{
		{ "models/p_angel.mdl",         "models/v_angel.mdl"   },
		{ "models/p_bush.mdl",          "models/v_bush.mdl"    },
		{ "models/p_cocker.mdl",        "models/v_cocker.mdl"  },
		{ "models/p_orracle.mdl",       "models/v_orracle.mdl" },
		{ "models/p_excal.mdl",         "models/v_excal.mdl"   },
		{ "models/p_timmy.mdl",         "models/v_timmy.mdl"    },
		{ "models/p_m98.mdl",           "models/v_m98.mdl"     },
		{ "models/p_mag.mdl",           "models/v_mag.mdl"     },
		{ "models/p_phantom.mdl",		"models/v_phantom.mdl" },
		{ "models/p_shocker.mdl",       "models/v_shocker.mdl" },
		{ "models/p_sl68.mdl",          "models/v_sl68.mdl"    },
		{ "models/p_spyder.mdl",        "models/v_spyder.mdl"  },
		{ "models/p_emag.mdl",			"models/v_emag.mdl"	   },
		{ "models/p_impulse.mdl",		"models/v_impulse.mdl" },
		{ "models/p_matrix.mdl",		"models/v_matrix.mdl"  },
		{ "models/p_omen.mdl",		"models/v_omen.mdl"  },
		{ "models/p_a5.mdl",		"models/v_a5.mdl"  },
		{ "models/p_blazer.mdl",		"models/v_blazer.mdl"  },
		{ "models/p_dm4.mdl",		"models/v_dm4.mdl"  },
		{ NULL, NULL } }; // kuja - sloppy jonah left this table un-updated, causing mass problems with new guns
...
/*
int V_FindWeaponByWeaponModel(int weaponindex)
{

	struct { char *name; int number; } modelmap[] =	{
		{ "models/p_angel.mdl",         WEAPON_ANGEL   },
		{ "models/p_bush.mdl",          WEAPON_BUSH    },
		{ "models/p_cocker.mdl",        WEAPON_COCKER  },
		{ "models/p_excal.mdl",         WEAPON_EXCAL   },
		{ "models/p_gren.mdl",          WEAPON_GREN    },
		{ "models/p_m98.mdl",           WEAPON_M98     },
		{ "models/p_mag.mdl",           WEAPON_MAG     },
		{ "models/p_pgp.mdl",           WEAPON_PGP     },
		{ "models/p_phantom.mdl",		WEAPON_PHANTOM },
		{ "models/p_pt.mdl",            WEAPON_PT      },
		{ "models/p_shocker.mdl",       WEAPON_SHOCKER },
		{ "models/p_sl68.mdl",          WEAPON_SL68    },
		{ "models/p_spyder.mdl",        WEAPON_SPYDER  },
		{ NULL, NULL } };
	...
	if ( weaponModel )
	{
		...
		while ( modelmap[i].name != NULL )
		{
			if ( !strnicmp( weaponModel->name, modelmap[i].name, len ) )
			{
				return ( modelmap[i].number );
			}
			i++;
		}
...
void V_CalcSpectatorRefdef ( struct ref_params_s * pparams )
{
	...
	if (  ( g_iUser1 == OBS_IN_EYE || gHUD.m_Spectator.m_pip->value == INSET_IN_EYE ) && ent )
	{
		...
		if ( lastWeaponModelIndex != ent->curstate.weaponmodel )
		{
			...
			{
				gEngfuncs.pfnWeaponAnim(2,0);	// reset weapon animation
			}
			else
			{
				// model not found
