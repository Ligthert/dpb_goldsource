...
void KeyDown (kbutton_t *b)
{
	...
	if (c[0])
		...
		/* Kuja - deprecated
		if(b==&in_attack) // Kill scripting for +attack
			return;
		*/
	}
...
{
	g_bAttackOK = false;
	if (pszCurrentBinding && !strcmp("+attack",pszCurrentBinding))
		g_bAttackOK = true;
	if (gViewPort)
...
void IN_DashDown(void) { KeyDown(&in_dash); }
void IN_DashUp(void) { KeyUp(&in_dash); }
...
{
	punch[0]-=gEngfuncs.pfnRandomFloat(0,f);
	punch[1]+=gEngfuncs.pfnRandomFloat(-f*0.5,f*0.5);
}
...
#define STAMINA_REST 17.5f
#define STAMINA_DRAIN 35.0f

extern int g_Buttons;
extern float v_speed;
extern "C" float csjump;
float csjump=-1.0f;
extern int v_ground;
extern "C" int iIsSpectator;
void DLLEXPORT CL_CreateMove ( float frametime, struct usercmd_s *cmd, int active )
{	
	float spd;
	vec3_t viewangles;
	...
	if ( active )
	{	
		float up=cl_pitchup->value, down=cl_pitchdown->value;
		if(g_iUser4&U_PRONING) {
			cl_pitchup->value=30.0f;
			cl_pitchdown->value=15.0f;
		}
		//memset( viewangles, 0, sizeof( vec3_t ) );
		//viewangles[ 0 ] = viewangles[ 1 ] = viewangles[ 2 ] = 0.0;
		gEngfuncs.GetViewAngles( (float *)viewangles );
		
		VectorAdd(punch,viewangles,viewangles);
		VectorClear(punch);

		CL_AdjustAngles ( frametime, viewangles );

		memset (cmd, 0, sizeof(*cmd));
		if(g_iUser1||gHUD.m_Timer.m_RoundState||(gViewPort && gViewPort->AllowedToPrintText()))
		...
		if(csjump>=0.0f) {
			float f=(1.5f-csjump)/1.5f;
			
			spd*=f*f;
		}
		if(v_ground||g_iUser1) {
			if(csjump==1.5f) 
				gHUD.m_Hopper.m_flStamina=max(0.0f,gHUD.m_Hopper.m_flStamina-30.0);
			csjump-=frametime;
		}		
		...
		if(((!g_iUser1) && (!gHUD.m_Timer.m_RoundState))|| (gViewPort && !gViewPort->AllowedToPrintText()) || g_iUser4&U_INPRONE )
{
			cmd->upmove=cmd->sidemove=cmd->forwardmove=0.0f;
						KeyUp(&in_left);
			KeyUp(&in_right);
		}
...
void InitInput (void)
{
	...
	gEngfuncs.pfnAddCommand ("prone",IN_ToggleProne);
	gEngfuncs.pfnAddCommand ("changeteam",ChangeTeam);
	gEngfuncs.pfnAddCommand ("changeconfig",ChangeConfig);
